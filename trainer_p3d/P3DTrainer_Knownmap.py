    ...
    def main_loop(self):
        ...
        for i_episode in range(self.config.num_episodes_to_run):
            ...
            while not done:
                ...
                action = self.agent.pick_action([global_map, observed_map, robot_pose_input])

                (global_map_next, observed_map_next, robot_pose_next), reward, unknown_cells, done, _ = self.field.step(
                    action)
                ...
                self.agent.step(state=[global_map, observed_map, robot_pose_input], action=action, reward=reward,
                                next_state=[global_map_next, observed_map_next, robot_pose_input_next], done=done)

                # to the next state
                observed_map = observed_map_next.copy()
                robot_pose = robot_pose_next.copy()
                global_map = global_map_next.copy()
                # trainer_p3d

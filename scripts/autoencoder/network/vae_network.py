...
class CompletionVAEShadowNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.enc_block_s1 = nn.Sequential(
            ME.MinkowskiConvolution(2, enc_ch[0], kernel_size=3, stride=1, dimension=3),
        ...
        self.dec_block_s8s4 = nn.Sequential(
            ME.MinkowskiGenerativeConvolutionTranspose(
                dec_ch[3], dec_ch[3], kernel_size=2, stride=2, dimension=3
            ),
    ...
    def forward(self, partial_in, target_key):
        ...
        print("out_global:", len(out_global))

        means = self.linear_mean(out_global)
        log_vars = self.linear_log_var(out_global)
        zs = means
        if self.training:
        ...
        z = ME.SparseTensor(
            ...
            coordinate_manager=zs.coordinate_manager,
        )

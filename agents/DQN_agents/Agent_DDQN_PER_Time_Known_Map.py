...
from memory.replay_buffer_knownmap_temporal import PriorityReplayBufferKnownMapTemporal
...
class Agent_DDQN_PER_Time_KnownMap(DDQN):
    ...
    def __init__(self, config):
        ...
        self.memory = PriorityReplayBufferKnownMapTemporal(buffer_size=self.hyper_parameters['buffer_size'],
    ...
    def pick_action(self, state):
        ...
        state = [torch.Tensor([known_map]).to(self.device), torch.Tensor([frame]).to(self.device),
    ...
    def update_memory_batch_errors(self, tree_idx, td_errors, rewards):
        ...
        loss_reward_each_item = 0.0001 * loss_each_item + rewards

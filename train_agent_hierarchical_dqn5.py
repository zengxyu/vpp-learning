...
field = Field(shape=(256, 256, 256), sensor_range=50, hfov=90.0, vfov=60.0, scale=0.05, max_steps=30,
...
def get_robot_pose_delta(robot_pose):
    thresh = 20
    pose_delta = np.random.randint(0, thresh, (3,))
...
for i_episode in range(params['num_episodes']):
    ...
    thresh = 20
    direction = np.random.randint(0, thresh, (3,))
    # direction = get_robot_pose_delta()
    pose_destination = robot_pose[:3] + direction[:3]
    actions = []
    ...
    while not done:
        ...
        reward2 = 34 - get_eu_distance(robot_pose_next[:3], pose_destination)
        reward2 = int(reward2)

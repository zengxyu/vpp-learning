...
def train_fun():
    ...
    out_folder = "test_out_p3d_static_env_action36"
    in_folder = "output_remote3/out_p3d_static_env_action36"
    # network
    ...
    config.set_parameters({"epsilon": 0.2})
    config.set_parameters({"epsilon_min": 0})
    #     config.set_parameters({"epsilon": 0.3})
    #     config.set_parameters({"epsilon_decay_rate": 0.98})
    #     config.set_parameters({"epsilon_min": 0})
    # Agent
    agent = Agent(config)

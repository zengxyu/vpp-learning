...
from network.network_dqn import DQN_Network
...
class Agent:
    def __init__(self, params, summary_writer, model_path=""):
        ...
        # 目标target
        self.policy_net = DQN_Network(self.action_size).to(self.device)
        self.target_net = DQN_Network(self.action_size).to(self.device)
        if not model_path == "":

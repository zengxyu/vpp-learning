...
class Agent:
        ...
        if len(self.frames) > self.seq_len:
            ...
            self.memory.put_data(
                [self.frames[:n], self.robot_poses[:n], self.actions[:n], self.rewards[:n], self.frames_prime[:n],
                 self.robot_poses_prime[:n], self.probs[:n], self.returns[:n], self.advantages[:n], self.dones])
    ...
    def train_net(self):
        ...
        frames, robot_poses, actions, rewards, frames_prime, robot_poses_prime, old_probs, returns, advantages, dones = self.memory.make_batch(
            self.train_device)
        v_prime, pol_prime = self.policy(robot_poses_prime)
        td_target = rewards + self.gamma * v_prime * (1 - dones)

        for i in range(self.K_epoch):
            vs, new_probs = self.policy(robot_poses)

            advantage_lst = []
            advantage = 0.0
            deltas = (td_target - vs).detach().cpu().numpy()
            ...
            loss_val = c1 * F.mse_loss(vs, returns)
    ...
    def act(self, frame, robot_pose):
        ...
        value, probs = self.policy(robot_pose_in)

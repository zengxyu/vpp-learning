...
from rl_agents.agents.HER_Base import HER_Base


class AgentDQNHER(HER_Base, AgentDQN):
    ...
    def __init__(self, config, network_cls, action_space):
        ...
        HER_Base.__init__(self, self.hyper_parameters["buffer_size"], self.hyper_parameters["batch_size"],
                          self.hyper_parameters["HER_sample_proportion"])
    ...
    def step(self):
        ...
        while not self.done:
            ...
            if self.time_for_q_network_to_learn():
                for _ in range(self.hyper_parameters["learning_iterations"]):

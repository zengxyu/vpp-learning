...
from rl_agents.agents.actor_critic_agents.sac import SAC


class SAC_Discrete(SAC):
    """The Soft Actor Critic for discrete action_space. It inherits from SAC for continuous action_space and only changes a few
    methods."""
    agent_name = "SAC"

    def __init__(self, config):
        BaseAgentAC.__init__(self, config)
        # assert self.action_types == "DISCRETE", "Action types must be discrete. Use SAC instead for continuous action_space"
        ...
        self.critic_target_2 = self.create_critic_network(state_dim=self.state_size,
                                                          action_dim=self.action_size, output_dim=1)
        BaseAgentAC.copy_model_over(self.critic_local, self.critic_target)
        BaseAgentAC.copy_model_over(self.critic_local_2, self.critic_target_2)

...
class PriorityReplayBufferKnownMapTemporal:
    """
    ...
    def transform(self, data_slice):
        ...
        for i in range(data_slice.shape[0]):
            known_map, frame, pose, action, reward, next_known_map, next_frame, next_pose, done = data_slice[i]
            frames_in.append(frame)
            robot_poses_in.append(pose)
            next_frames_in.append(next_frame)
            next_robot_poses_in.append(next_pose)
        cur_known_map, cur_frame, cur_pose, cur_action, cur_reward, cur_next_known_map, cur_next_frame, cur_next_pose, cur_done = \
            data_slice[-1]

        known_map_in = cur_known_map
        frames_in = np.array(frames_in, dtype=np.float)
        robot_poses_in = np.array(robot_poses_in, dtype=np.float)
        action = cur_action
        reward = cur_reward
        next_known_map_in = cur_next_known_map
        next_frames_in = np.array(next_frames_in, dtype=np.float)
        next_robot_poses_in = np.array(next_robot_poses_in, dtype=np.float)
        done = cur_done

        return np.array([known_map_in, frames_in, robot_poses_in, action, reward, next_known_map_in, next_frames_in,
                         next_robot_poses_in, done], dtype=np.object)

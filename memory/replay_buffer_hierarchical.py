...
class ManagerPriorityReplayBuffer:
    """
    ...
    def add_experience(self, state, reward, next_state, done):
    ...
    def sample(self, num_experiences=None):
        ...
        for i in range(batch_size):
            ...
            ISWeights[i, 0] = np.power(prob / (min_prob + 1e-16), -self.beta)
    ...
    def to_tensor(self, mini_batch):
        ...
        # goal
        actions = self.to_float_tensor(actions).squeeze(1)
    ...
    def batch_update(self, tree_idx, abs_errors):
        ...
        clipped_errors = np.clip(np.minimum(abs_errors, self.abs_err_upper), 0, np.inf)
...
class WorkerPriorityReplayBuffer:
    """
    A special replay buffer which stores a priority for each event and only overwrites the ones with lowest priority

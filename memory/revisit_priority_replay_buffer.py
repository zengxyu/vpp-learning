...
class PriorityReplayBuffer:
    ...
    def to_tensor(self, mini_batch, is_vpp):
        ...
        if is_vpp:
            visit_map_in, frames_in, robot_poses_in, actions, rewards, next_visit_map_in, next_frames_in, next_robot_poses_in, dones = mini_batch
            visit_map_in = self.to_float_tensor(visit_map_in)
            frames_in = self.to_float_tensor(frames_in)
            robot_poses_in = self.to_float_tensor(robot_poses_in)

            next_visit_map_in = self.to_float_tensor(next_visit_map_in)
            next_frames_in = self.to_float_tensor(next_frames_in)
            next_robot_poses_in = self.to_float_tensor(next_robot_poses_in)
            state_in = [visit_map_in, frames_in, robot_poses_in]
            next_state_in = [next_visit_map_in, next_frames_in, next_robot_poses_in]
        else:

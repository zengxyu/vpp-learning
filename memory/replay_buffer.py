...
from memory.data_structures import SumTree, DoubleSumTree
...
class DoublePriorityReplayBuffer:
    """
    A special replay buffer which stores a priority for each event and only overwrites the ones with lowest priority
    ...
    def __init__(self, buffer_size, batch_size, device, is_discrete, seed=1337):
        self.tree = DoubleSumTree(buffer_size)
        self.buffer_size = buffer_size
        self.size = 0
        self.batch_size = batch_size
        self.device = device
    ...
    def add_experience(self, state, action, reward, next_state, done):
        ...
        # reward[1] known
        transition = np.array(
            [state[0], state[1], action, reward[0], reward[1], next_state[0], next_state[1], done],
            dtype=np.object)
        max_p_unknown = np.max(self.tree.tree_unknown[-self.tree.capacity:])
        max_p_known = np.max(self.tree.tree_known[-self.tree.capacity:])

        if max_p_unknown == 0.0:
            max_p_unknown = self.abs_err_upper
        if max_p_known == 0.0:
            max_p_known = self.abs_err_upper

        self.tree.add(max_p_unknown, max_p_known, transition)  # set the max p for new p
        self.size += 1

    def sample_from_unknown(self, is_vpp=True, num_experiences=None, is_weighted=True):
        # Draws a random sample of experience from the replay buffer
        batch_size = self.batch_size if num_experiences is None else num_experiences
        b_idx, b_memory, ISWeights = np.empty((batch_size,), dtype=np.int32), np.empty(
            (batch_size, self.tree.data[0].size), dtype=object), np.empty((batch_size, 1), dtype=np.float)
        pri_seg = self.tree.total_p_unknown / float(batch_size)  # priority segment
        self.beta = np.min([1., self.beta + self.beta_increment_per_sampling])  # max = 1

        if self.size > self.tree.capacity:
            min_prob = np.min(self.tree.tree_unknown[-self.tree.capacity:]) / self.tree.total_p_unknown
        else:
            min_prob = np.min(self.tree.tree_unknown[
                              self.tree.capacity - 1:self.tree.capacity - 1 + self.size]) / self.tree.total_p_unknown  # for later calculate ISweight
        for i in range(batch_size):
            a, b = pri_seg * i, pri_seg * (i + 1)
            v = random.uniform(a, b)
            idx, p, data_idx, data = self.tree.get_unknown_leaf(v)
            prob = p / self.tree.total_p_unknown
            ISWeights[i, 0] = np.power(prob / min_prob, -self.beta)
            b_idx[i], b_memory[i, :] = idx, data
    ...
    def sample_from_known(self, is_vpp=True, num_experiences=None, is_weighted=True):
        # Draws a random sample of experience from the replay buffer
        batch_size = self.batch_size if num_experiences is None else num_experiences
        b_idx, b_memory, ISWeights = np.empty((batch_size,), dtype=np.int32), np.empty(
            (batch_size, self.tree.data[0].size), dtype=object), np.empty((batch_size, 1), dtype=np.float)
        pri_seg = self.tree.total_p_known / float(batch_size)  # priority segment
        self.beta = np.min([1., self.beta + self.beta_increment_per_sampling])  # max = 1

        if self.size > self.tree.capacity:
            min_prob = np.min(self.tree.tree_known[-self.tree.capacity:]) / self.tree.total_p_known
        else:
            min_prob = np.min(self.tree.tree_known[
                              self.tree.capacity - 1:self.tree.capacity - 1 + self.size]) / self.tree.total_p_known  # for later calculate ISweight
        for i in range(batch_size):
            a, b = pri_seg * i, pri_seg * (i + 1)
            v = random.uniform(a, b)
            idx, p, data_idx, data = self.tree.get_known_leaf(v)
            prob = p / self.tree.total_p_known
            ISWeights[i, 0] = np.power(prob / min_prob, -self.beta)
            b_idx[i], b_memory[i, :] = idx, data
    ...
    def to_tensor(self, mini_batch, is_vpp):
        mini_batch = mini_batch.T
        frames_in, robot_poses_in, actions, rewards_unknown, rewards_known, next_frames_in, next_robot_poses_in, dones = mini_batch
        frames_in = self.to_float_tensor(frames_in)
        robot_poses_in = self.to_float_tensor(robot_poses_in)
        ...
        actions = self.to_float_tensor(actions)
        rewards_unknown = self.to_float_tensor(rewards_unknown)
        rewards_known = self.to_float_tensor(rewards_known)
        dones = self.to_long_tensor(dones)
        if self.is_discrete:
            actions = actions.long()
        if len(actions.shape) == 1:
            actions = actions.unsqueeze(1)
        if len(rewards_unknown.shape) == 1:
            rewards_unknown = rewards_unknown.unsqueeze(1)
        if len(rewards_known.shape) == 1:
            rewards_known = rewards_known.unsqueeze(1)
        if len(dones.shape) == 1:
            dones = dones.unsqueeze(1)
        rewards = [rewards_unknown, rewards_known]
        return [state_in, actions, rewards, next_state_in, dones]
    ...
    def batch_update_unknown(self, tree_idx, abs_errors):
        # Update the priority of the given index
        abs_errors += self.epsilon  # convert to abs and avoid 0
        clipped_errors = np.minimum(abs_errors, self.abs_err_upper)
        ps = np.power(clipped_errors, self.alpha)
        for ti, p in zip(tree_idx, ps):
            self.tree.update_unknown(ti, p)

    def batch_update_known(self, tree_idx, abs_errors):
        # Update the priority of the given index
        abs_errors += self.epsilon  # convert to abs and avoid 0
        clipped_errors = np.minimum(abs_errors, self.abs_err_upper)
        ps = np.power(clipped_errors, self.alpha)
        for ti, p in zip(tree_idx, ps):
            self.tree.update_known(ti, p)

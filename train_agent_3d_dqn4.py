...
from field_joint_ros import Field, Action
from network.network_dqn import DQN_Network6, DQN_Network8, DQN_Network9, DQN_Network11, DQN_Network12, DQN_Network13
...
field = Field(shape=(256, 256, 256), sensor_range=50, hfov=90.0, vfov=60.0, scale=0.05, max_steps=100,
...
def main_loop():
    ...
    observed_map, robot_pose, robot_joint_pose = field.reset()
    ...
    for i_episode in range(params['num_episodes']):
        ...
        while not done:
            ...
            robot_pose_input = np.concatenate([robot_pose[:3], robot_direction.squeeze(), robot_joint_pose], axis=0)

            action = player.act(observed_map, robot_pose_input)
            print()
            time3 = time.time()
            observed_map_next, robot_pose_next, robot_joint_pose_next, reward1, done = field.step(action)
            print(
            ...
            # diff direction
            robot_pose_input_next = np.concatenate(
                [robot_pose_next[:3], robot_direction_next.squeeze(), robot_joint_pose_next], axis=0)
            ...
            robot_joint_pose = robot_joint_pose_next.copy()
            # train
            ...
            if done:
                ...
                print("robot pose: {}".format(robot_joint_pose[:3]))
                ...
                player.reset()
                observed_map, robot_joint_pose = field.reset()
                rewards1 = []
                rewards2 = []

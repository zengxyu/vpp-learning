...
"""
worker 单独训练，
manager的critic network输出参数可以不用frame, 只需要 predicted direction(action), robot pose
"""
...
params = {
    ...
    # output
    'output_folder': "output_hierarchical_03",
    'log_folder': 'log',
    'model_folder': 'model',
...
for i_episode in range(params['num_episodes']):
    ...
    initial_rotation_direction = np.array([[-1], [0], [0]])
    ...
    manager_rewards2 = []
    while not done:
        pose_direction = direction[:3]
        # rotation
        # normalize direction
        unit_direction = pose_direction / (np.linalg.norm(pose_direction) + 1e-8)
        # robot direction
        rotation_direction = Rotation.from_quat(robot_pose[3:]).as_matrix() @ initial_rotation_direction

        diff_rotation_direction = unit_direction - rotation_direction.squeeze()
        diff_rotation_directions.append(int(np.linalg.norm(diff_rotation_direction) * 100) / 100)

        robot_pose_input = np.concatenate([pose_direction, diff_rotation_direction, robot_pose], axis=0)
        action = player.act(robot_pose_input)

        observed_map_next, robot_pose_next, reward1, reward3, done = field.step(action)

        # 这里构造奖励
        pose_destination = robot_pose[:3] + pose_direction
        reward2 = 109 - get_eu_distance(robot_pose_next[:3], pose_destination)
        reward2 = int(reward2)

        reward_distance = 1 if np.linalg.norm(pose_direction) < 30 else -np.linalg.norm(pose_direction)
        reward = reward2
        manager_reward1 += reward1
        manager_reward2 += reward_distance

        # 构造下一个状态
        pose_direction_next = pose_destination - robot_pose_next[:3]
        # normalize direction next
        unit_direction_next = pose_direction_next / np.linalg.norm(pose_direction_next)
        # robot direction next
        robot_rotation_direction_next = Rotation.from_quat(robot_pose_next[3:]).as_matrix() @ initial_rotation_direction
        # diff direction next
        diff_rotation_direction_next = unit_direction_next - robot_rotation_direction_next.squeeze()

        robot_pose_input_next = np.concatenate([pose_direction_next, diff_rotation_direction_next, robot_pose_next],
                                               axis=0)
        ...
        if time_step_episode % params['c_freq'] == 0:
            destination_list.append(np.append(pose_destination.tolist(), robot_pose[:3]))

            manager_observed_map, manager_robot_pose = observed_map, robot_pose

        if (time_step_episode + 1) % params['c_freq'] == 0:
            manager_rewards1.append(manager_reward1)
            manager_rewards2.append(manager_reward2)
            manager_reward = manager_reward1 + manager_reward2
            player.store_manager_experience(state=[manager_observed_map, manager_robot_pose, np.array(direction)],
        ...
        if done:
            ...
            print("manager_rewards1:{}".format(manager_rewards1))
            print("manager_rewards2:{}".format(manager_rewards2))

            print("actions:{}".format(actions))
            print("diff directions norm:{}".format(diff_rotation_directions))

            print("diff_direction_next:{}".format(diff_rotation_direction_next))

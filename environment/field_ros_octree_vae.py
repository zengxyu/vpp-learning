...
class Field:
    def __init__(self, config, Action, shape, sensor_range, hfov, vfov, max_steps, move_step, handle_simulation):
        ...
        ae_args = get_parse_args()
        self.ae_learner = AELearner(ae_args)
    ...
    def step(self, action):
        ...
        voxelgrid, robotPose, robotJoints, reward = self.client.sendRelativePose(relative_pose)
        features = self.ae_learner.inference(voxelgrid)
        ...
        return (features, robotPose), reward, done, {}
    ...
    def reset(self, is_sph_pos, is_global_known_map, is_egocetric, is_randomize, randomize_control, last_targets_found):
        ...
        self.found_targets = 0
        self.free_cells = 0
        voxelgrid, robotPose, robotJoints, reward = self.client.sendReset(map_type='voxelgrid')
        features = self.ae_learner.inference(voxelgrid)

        return (features, robotPose)
